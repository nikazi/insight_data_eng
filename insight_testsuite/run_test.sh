...
function compare_outputs {
  ...
  PROJECT_ANSWER_PATH1=../output/${OUTPUT_FILENAME}
  TEST_ANSWER_PATH1=${GRADER_ROOT}/tests/${test_folder}/output/${OUTPUT_FILENAME}

  # Ignore line order when comparing.
  DIFF_RESULT1=$(diff -bB <(sort ${PROJECT_ANSWER_PATH1}) <(sort ${TEST_ANSWER_PATH1}) | wc -l)
  ...
  if [ "${DIFF_RESULT1}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH1} ]; then
    ...
    diff <(sort ${PROJECT_ANSWER_PATH1}) <(sort ${TEST_ANSWER_PATH1})
  fi
...
function run_all_tests {
  ...
  for test_folder in ${TEST_FOLDERS}; do
	...
	cd ../
    bash run.sh ./insight_testsuite/tests/${test_folder} ./ 2>&1
